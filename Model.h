...
struct FlagErr {
  union {
    uint16_t flag;
    struct {
      uint16_t fErrorEE : 1;     //Indica Error por EEPROM Interna ESP
      uint16_t fErrorFlash : 1;  //Indica Error por Memoria Flash
      uint16_t fErrorSD : 1;     //Indica Error por Memoria SD
...
struct FlagFact {
  union {
    uint16_t flagFact;
    struct {
      uint16_t flagInfTituloSeniat:1; //si = 1 Se Registro el Titulo SENIAT
...
extern char chG_SerialPtr[CantMaxCarConf];                                  //Se Almacena el Serial de la Impresora ej. SS00000001 (desde archivo configParam.json)
