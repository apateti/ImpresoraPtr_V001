...
char chG_SerialPtr[CantMaxCarConf];
...
void setup() {
  if (boInicializarHardware() == ERROR) {
    Serial.println("ERROR, No puede continuar por error en Hardware");
  }
  fInicializaFactura();
  inicializaAcumuladores();
  if(boCompruebaCambio_eMMC()== ERROR){
    Serial.println("ERROR, memoria eMMC cambiada");
  }
  RtcDateTime timeRTC;
  timeRTC = rtc1302.GetDateTime();
  uint32_t timeNowUnix = myFunciones.timeToUnix(timeRTC);
  if((timeNowUnix - stcG_Contadores.ui32FechaIniRep) > Segundos24H){
    Serial.println("Inicializando Reportes de X y Z");
    //generaReporteZ();
  }
}
...
uint8_t verificar_FLASH(void){
  ...
  if (!SerialFlash.begin(spiFlash, CSPIN)) {
    Serial.println(F("ERROR to access SPI Flash chip"));
    return (ERROR_FLASH);
  } else {
...
bool bValidoDatos_cmd() {
  ...
  if (doc["cmd"] == CMD_Encabezado) {  //El comando "cmd" es Encabezado?
    //strncpy(chG_CmdRx, CMD_Encabezado, CaracMaxCmdTx);
    strncpy(stcG_CmdTxJson.chrG_CmdTx, CMD_Encabezado, CaracMaxCmdTx);
    if (!doc["data"]) {
  ...
  } else if (doc["cmd"] == CMD_Cliente) {  //El comando "cmd" es Cliente?
    //chG_CmdRx = "cli";
    //strncpy(chG_CmdRx, CMD_Cliente, CaracMaxCmdTx);
    strncpy(stcG_CmdTxJson.chrG_CmdTx, CMD_Cliente, CaracMaxCmdTx);
    if (!doc["data"]) {
      //byG_CodeTx = ErrorDatBlank;
      stcG_CmdTxJson.ui8CodeTx = ErrorDatBlank;
      stcG_FlagFact.flagTxRespuesta = 1;  //Se Tx la Respuesta al HOST
  ...
  } else if (doc["cmd"] == CMD_Comentario) {  //El comando "cmd" es Comoentarios?
    //chG_CmdRx = "com";
    //strncpy(chG_CmdRx, CMD_Comentario, CaracMaxCmdTx);
    strncpy(stcG_CmdTxJson.chrG_CmdTx, CMD_Comentario, CaracMaxCmdTx);
    if (!doc["data"]) {
      //byG_CodeTx = ErrorDatBlank;
      stcG_CmdTxJson.ui8CodeTx = ErrorDatBlank;
      stcG_FlagFact.flagTxRespuesta = 1;  //Se Tx la Respuesta al HOST
  ...
  } else if (doc["cmd"] == CMD_Producto) {  //El comando "cmd" es Productos?
#ifdef timeDebug
    long iniTime;
    iniTime = millis();
#endif
    ...
    strncpy(stcG_CmdTxJson.chrG_CmdTx, CMD_Producto, CaracMaxCmdTx);
    if (!doc["data"]) {
      //byG_CodeTx = ErrorDatBlank;
      stcG_CmdTxJson.ui8CodeTx = ErrorDatBlank;
      stcG_FlagFact.flagTxRespuesta = 1;  //Se Tx la Respuesta al HOST
    ...
    //chG_CmdRx = "time";
    //strncpy(chG_CmdRx, CMD_setTimer, CaracMaxCmdTx);
...
//#else
    ...
    // if (data["epoch"]) {
    //   long data = data["epoch"];
    //   timeRTC = RtcDateTime(data);
    //   rtc1302.SetDateTime(timeRTC);
    // }
...
//#endif
    ...
    if(doc["data"] == 1){
      ...
      //Serial.print("Se enviara RTC time EPOCH: ");
      //Serial.print(datestring);
      // struct tm time_now_tm;
      // time_now_tm.tm_mday = timeRTC.Day();
      // time_now_tm.tm_mon = timeRTC.Month()-1;
      // time_now_tm.tm_year = timeRTC.Year()-1900;
      // //
      // time_now_tm.tm_hour = timeRTC.Hour();
      // time_now_tm.tm_min = timeRTC.Minute();
      // time_now_tm.tm_sec = timeRTC.Second();
      ...
      // uint32_t time_now_epoch = mktime(&time_now_tm);
      //unsigned long time_now_epoch = unixtime(time_now_tm);
      //strncpy(stcG_CmdTxJson.chrG_DataTx,time_now_epoch,CantMaxDataRx);
  ...
  else if (doc["cmd"] == CMD_FormPago) {  //Entrada Forma de Pago
    //chG_CmdRx = "fpa";
    //strncpy(chG_CmdRx, CMD_FormPago, CaracMaxCmdTx);
    strncpy(stcG_CmdTxJson.chrG_CmdTx, CMD_FormPago, CaracMaxCmdTx);
    if (!doc["data"]) {
      //byG_CodeTx = ErrorDatBlank;
      stcG_CmdTxJson.ui8CodeTx = ErrorDatBlank;
      stcG_FlagFact.flagTxRespuesta = 1;  //Se Tx la Respuesta al HOST
  ...
  } else if (doc["cmd"] == CMD_EndProducto) {
    //strncpy(chG_CmdRx, CMD_EndProducto, CaracMaxCmdTx);
    strncpy(stcG_CmdTxJson.chrG_CmdTx, CMD_EndProducto, CaracMaxCmdTx);
    stcG_currentInput = Input::InpFinProd;
  } else if (doc["cmd"] == CMD_LineAdicionales) {
    if (!doc["data"]) {
      //byG_CodeTx = ErrorDatBlank;
      stcG_CmdTxJson.ui8CodeTx = ErrorDatBlank;
      return ErrorYes;
    ...
    //strncpy(chG_CmdRx, CMD_EndFactura, CaracMaxCmdTx);
    ...
    //strncpy(chG_CmdRx, CMD_Desconocido, CaracMaxCmdTx);
    strncpy(stcG_CmdTxJson.chrG_CmdTx, CMD_Desconocido, CaracMaxCmdTx);
    //byG_CodeTx = ErrorCmdUnk;           //Se inicializa los comandos a Tx al HOST
...
void outputLinAdic(void) {
  ...
  if (byG_NroLinAdicionales > 9)
    byG_NroLinAdicionales = 9;
...
uint8_t fImprimirFactura(void) {
...
#endif
  int i;
  //char chL_GuardaFile[CantMaxCarLin * MultCantCarFile];  //Variable para Almacenar temporal datos que se
  //  grabaran en memoria eMMC
  // *strG_FacturaOut = '\n';
  //strG_FacturaOut = '\n';
  chG_ImpPtr[0] = '\0';
...
#ifdef useDebug
  Serial.print("Archivo de Trabajo de la SD: ");
  Serial.println(dirFileMT);
#endif
  ...
  stcG_XyZ.stContadores.ui32FechaUF = myFunciones.timeToUnix(timeRTC);
  //stcG_XyZ.stContadores.ui32CountFacDia++;
  //*****************************************************************************
...
uint8_t boCompruebaCambio_eMMC(void){
